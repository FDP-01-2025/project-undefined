...
bool playKeySmash(int posX, int posY)
{
    ...
    while (bossHP > 0)
    {
        int numKeys = 4 + currentRound- 1;
        int scoreToWin = 30 + (currentRound- 1) * 10; // Score needed to win the round
        // Initialize starting speed
        int speed = 350 - (currentRound- 1) * 40; // Speed decreases as rounds increase
        if (speed < 80)
        {
        ...
        showAnimatedMessage("<<<< SMASH KEY - RONDA " + to_string(currentRound) + " >>>>" 
        ...
        setColor(6);
        cout << "    <<<<< 𝑺𝒎𝒂𝒔𝒉 𝒌𝒆𝒚 - " << "𝑹𝒐𝒏𝒅𝒂: " << currentRound<< " >>>>>";
        setColor(15);
